...
public class AssetUserIssuedList extends AssetList {
	private static AssetUserIssuedList instance;
	public static AssetUserIssuedList getInstance() {
		if (!isInstance())
			instance = new AssetUserIssuedList();
		return instance;
	}
	public static boolean isInstance() {
		return (instance != null);
	}
	private AssetUserIssuedList() {
	}
	
	@Override
	public String getName() {
		return "User-Issued Assets";
	}
	...
	protected TableView<Asset> createPaginationTableView() {
		...
		TableColumn<Asset, Asset> column04 = new TableColumn<Asset, Asset>();
		column04.setText("Issuer");
		column04.setPrefWidth(150.0);
		column04.setMinWidth(120.0);
		column04.setCellValueFactory(item -> new SimpleObjectProperty<Asset>(item.getValue()));
		...
		column04.setCellFactory(item -> {
			TableCell<Asset, Asset> cell = new TableCell<Asset, Asset>() {
                ...
                public void updateItem(Asset item, boolean empty) {
                	...
                	setText(null);
                    if (item != null && !empty) {
                    	Account account = getAccount(item.getIssuer_account_id());
                    	CellHyperlink link = new CellHyperlink();
                    	link.setText(account.getName());
                    	link.setOnMouseClicked(event -> {
