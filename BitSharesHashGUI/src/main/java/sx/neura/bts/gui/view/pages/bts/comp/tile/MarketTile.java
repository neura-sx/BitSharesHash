...
public class MarketTile extends Tile<Market> {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		if (item != null) {
			Asset quoteAsset = getAssetById(item.getQuote_id());
			Asset baseAsset = getAssetById(item.getBase_id());
			iconUI.setText(getMarketLabel(quoteAsset, baseAsset));
			...
			bidDepthUI.setText(getAmount(0, item.getBid_depth()));
			askDepthUI.setText(getAmount(0, item.getAsk_depth()));
