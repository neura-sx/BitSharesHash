...
public class MyOrderBidAskTile extends Tile<MyOrder> {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		if (item != null) {
			...
			priceUI.setText(String.format("%.8f", getRealPrice(item.getOrder().getMarket_index().getOrder_price())));
			
			quantityUI.setLabel(String.format("%s (%s)", "Quantity", assetBase.getSymbol()));
			if (item.getOrder().getType().equals("bid_order"))
				quantityUI.setText(getAmount(assetBase, (long) (item.getOrder().getState().getBalance() / item.getOrder().getMarket_index().getOrder_price().getRatio())));
			else if (item.getOrder().getType().equals("ask_order"))
				quantityUI.setText(getAmount(assetBase, item.getOrder().getState().getBalance()));
			
			totalUI.setLabel(String.format("%s (%s)", "Total", assetQuote.getSymbol()));
			if (item.getOrder().getType().equals("bid_order"))
				totalUI.setText(getAmount(assetQuote, item.getOrder().getState().getBalance()));
			else if (item.getOrder().getType().equals("ask_order"))
				totalUI.setText(getAmount(assetQuote, (long) (item.getOrder().getState().getBalance() * item.getOrder().getMarket_index().getOrder_price().getRatio())));
	...
	private void removeItself() {
		...
		if (item.getOrder().getType().equals("bid_order")) {
			...
			v += String.format("%s %s/%s\n", String.format("%.8f",
					getRealPrice(item.getOrder().getMarket_index().getOrder_price())), assetQuote.getSymbol(), assetBase.getSymbol());
			v += String.format("%s %s\n", assetBase.getSymbol(), getAmount(item.getOrder().getMarket_index().getOrder_price().getBase_asset_id(),
